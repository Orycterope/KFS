...
pub fn init_gdt() {
    ...
    GDT.call_once(|| {
        ...
        fault_task.init();
        let fault_task_stack_end = &DOUBLE_FAULT_TASK_STACK.0 as *const u8 as usize + size_of::<DoubleFaultTaskStack>();
        ...
        info!("Loaded GDT {:#?}\ncs: {:?}\nds: {:?}\nes: {:?}\nfs: {:?}\ngs: {:?}\nss: {:?}\nldt: {:?}\ntss: {:?}", gdt.deref().table, cs, ds, ds, gs, gs, ss, ldt_ss, tss_ss);
...
static DOUBLE_FAULT_TASK_STACK: DoubleFaultTaskStack = DoubleFaultTaskStack([0u8; PAGE_SIZE]);
